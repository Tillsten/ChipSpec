...
class Chip:
    ...
    chip_shape: tuple[int, int] = (9, 9)
    chip_spacing: dimension = (0.3, 0.3)
    feature_grid_shape: tuple[int, int] = (12, 12)
    feature_grid_size: dimension = 2, 2
    ...
    def get_compartment_position(self, xg: int, yg: int) -> dimension:
        ...
        x = (
            self.feature_grid_size[0] * xg
            + xg * self.chip_spacing[0]
            + self.border_spacing[0]
        )
        y = (
            self.feature_grid_size[1] * yg
            + yg * self.chip_spacing[1]
...
def draw_chip(chip: Chip):
    ...
    for xg, yg in np.ndindex(chip.chip_shape):
        ...
        ax.add_patch(
            plt.Rectangle(
                (x_pos, y_pos), *chip.feature_grid_size, color="black", alpha=0.5
            )
        )

def draw_comparment(chip: Chip, xg: int, yg: int):
    fig, ax = plt.subplots(subplot_kw=dict(aspect="equal"), figsize=(10, 10))
    ax.set_xlim(0, chip.feature_grid_size[0])
    ax.set_ylim(0, chip.feature_grid_size[1])
...
if __name__ == "__main__":
    ...
    chip = Chip()
    print(chip.feature_grid_shape[0] * (chip.feature_size[0] + chip.feature_spacing[0]))
    draw_chip(chip)
